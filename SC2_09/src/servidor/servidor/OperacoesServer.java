...
public class OperacoesServer {
	...
	private void copiarFotos(String userCurr, String userID) {
		String fotoPath = DiretoriosServidor.DIRETORIO_SERVIDOR + userID + File.separator;
		...
		for (Foto foto : fotos) {
			...
			Cifrar.mudarNomeFiles(fotoPath + foto.getNome(), fotoPath + foto.getNome() + ".temp");
	
			Cifrar.decifrarFicheiro(fotoPath + foto.getNome() + ".temp", fotoPath + foto.getNome(),
					fotoPath + foto.getNome() + ".key");
			

			ligacao.enviarImagem(fotoPath + foto.getNome());

			Cifrar.apagarFile(fotoPath + foto.getNome());
		

			Cifrar.mudarNomeFiles(fotoPath + foto.getNome() + ".temp", fotoPath + foto.getNome());
	...
	private void adicionarFoto(Mensagem msg) {
		...
		if (!existeFoto) {
			...
			String diretorio = DiretoriosServidor.DIRETORIO_SERVIDOR + userCurr + "/";
			Foto foto = ligacao.receberImagem(userCurr, nomeFoto, diretorio);
			BaseDados.getInstance().adicionarFoto(userCurr, foto);

			Cifrar.cifrarFicheiro(diretorio + nomeFoto, diretorio + nomeFoto + ".temp");
			Cifrar.apagarFile(diretorio + nomeFoto);
			Cifrar.mudarNomeFiles(diretorio + nomeFoto + ".temp", diretorio + nomeFoto);
